...
* **`sqlite-mcp-server`**: Allows the AI to query the local SQLite database directly to verify session saves.
* **`firebase-mcp-server`**: Enables the AI to query Firestore and Firebase Auth for cloud-synced data and user authentication.
...
Force the AI into this exact folder structure to prevent overcomplication:
    ...
    /Sentinel
      ...
      └── /Sentinel.Engine  (C# .NET 8 WPF Application + WebView2 + SQLite Core + Firebase)
...
> 5. Have C# listen for this message and write it to a local SQLite database using Entity Framework Core, and (if enabled) sync the distraction to Firestore.
> 6. Ensure all session and distraction data is mirrored to Firestore for users who have enabled cloud sync and authenticated via Firebase Auth.
